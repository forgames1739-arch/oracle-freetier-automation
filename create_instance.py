...
if __name__ == "__main__":
    logger.info("🚀 Запуск рандомного таймера для бесплатного аккаунта Oracle...")
    
    while True:
        create_instance()
        
        # === НОВОЕ: рандомный интервал ожидания (сам ждёт и выключается) ===
        wait_minutes = random.randint(5, 10)
        logger.info(f"⏳ Ждём {wait_minutes} минут и повторяем цикл (GitHub не тратит минуты!)")
